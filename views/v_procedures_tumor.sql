...
SELECT
    ...
    TRY(date_parse(p.performed_date_time, '%Y-%m-%d')) as proc_performed_date_time,
    TRY(date_parse(p.performed_period_start, '%Y-%m-%dT%H:%i:%sZ')) as proc_performed_period_start,
    TRY(date_parse(p.performed_period_end, '%Y-%m-%dT%H:%i:%sZ')) as proc_performed_period_end,
    TRY(date_parse(p.performed_string, '%Y-%m-%d')) as proc_performed_string,
