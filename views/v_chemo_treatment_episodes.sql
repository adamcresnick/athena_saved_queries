...
medications_with_episode_dates AS (
    SELECT
        ...
        COALESCE(
            TRY(date_parse(medication_start_date, '%Y-%m-%dT%H:%i:%sZ')),      -- 99.3% coverage
            TRY(date_parse(medication_authored_date, '%Y-%m-%dT%H:%i:%sZ'))     -- 100% fallback
        ) AS episode_start_datetime,

        -- Stop date LEFT AS NULL if not available (for uniform querying)
        TRY(date_parse(medication_stop_date, '%Y-%m-%dT%H:%i:%sZ')) AS episode_stop_datetime,
